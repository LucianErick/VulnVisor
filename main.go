...
var (
	tempJsonFileName = "vulnvisor-report-temp.json"
	version          = "dev"
...
func helpMessage() {
	_, err := fmt.Printf(`
vulnvisor v%s
Usage: trivy vulnvisor [-h,--help] command target filename
 A Trivy plugin that scans and output the results to a html file.
Options:
  -h, --help    Show usage.
Examples:
  # Scan 'alpine:latest' image
  trivy vulnvisor image alpine:latest --html-result result.html
  # Scan local folder
  trivy vulnvisor fs . --html-result result.html
`, version)
	if err != nil {
